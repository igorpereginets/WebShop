		...
		<div class="row">
			<div class="col-sm-12">
				<h1 class="page-header">
					States <small>Overview</small>
				</h1>
			</div>
		</div>
		<div class="col-sm-12">
			<form:form action="/admin/states" modelAttribute="filter" method="get" name="searchForm">
				<custom:hiddenInputs excludeParams="name" />
				<div class="row">
					<div class="col-md-11 col-sm-11">
						<form:input type="search" path="name" class="form-control" placeholder="Search state..." />
					</div>
			...
			<div class="panel panel-default">
				<div class="panel-heading">
					<h3 class="panel-title">
						<i class="fa fa-table fa-fw"></i> State Panel
					</h3>
				</div>
				<div class="panel-body">
					<div class="table-responsive">
						<table class="table table-bordered table-hover table-striped">
							...
							<tbody>
								<c:forEach items="${page.content}" var="state">
									<tr>
										<td>${state.id}</td>
										<td>${state.name}</td>
										<td><a href="/admin/states/update/${state.id}?${pageContext.request.queryString}" class="fa fa-fw fa-pencil"></a></td>
										<td><a href="/admin/states/delete/${state.id}?${pageContext.request.queryString}" class="glyphicon glyphicon-remove-sign"></a></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
					</div>
					<button type="button" class="btn btn-primary btn-md pull-right" data-toggle="modal" data-target="#add">Add State</button>
				</div>
			</div>
		</div>
		<div class="row">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="form-style-8">
					<form:form modelAttribute="stateSaveForm" action="/admin/states" method="post" id="save">
						<form:hidden path="id" />
						<form:errors path="name" />
						<form:input type="text" path="name" placeholder="State" required="true" maxlength="25" />
					</form:form>
				</div>
			</div>
			<div class="modal-footer">
				<button type="submit" class="btn btn-primary" form="save" formaction="/admin/states?${pageContext.request.queryString}">
					<span class="glyphicon glyphicon-ok"></span> Save
				</button>
				<c:choose>
					<c:when test="${stateSaveForm.id ne 0}">
						<a href="/admin/states?${pageContext.request.queryString}" id="close" class="btn btn-default">Close</a>
					</c:when>
					<c:otherwise>
...
<c:choose>
	...
	<c:otherwise>
		<spring:hasBindErrors name="stateSaveForm">
