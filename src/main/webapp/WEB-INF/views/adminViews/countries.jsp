		...
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header">
					Countries <small>Overview</small>
				</h1>
			</div>
		</div>
		<div class="col-sm-12">
			<form:form action="/admin/countries" modelAttribute="filter" method="get" name="searchForm">
				<custom:hiddenInputs excludeParams="name" />
				<form:input type="search" path="name" class="form-control" placeholder="Search country..." />
			</form:form>
			<div class="panel panel-default">
				<div class="panel-heading">
					<h3 class="panel-title">
						<i class="fa fa-table fa-fw"></i> Country Panel
					</h3>
				</div>
							...
							<tbody>
								<c:forEach items="${page.content}" var="country">
									<tr>
										<td>${country.id}</td>
										<td>${country.name}</td>
										<td><a href="/admin/countries/update/${country.id}?${pageContext.request.queryString}" class="fa fa-fw fa-pencil"></a></td>
										<td><a href="/admin/countries/delete/${country.id}?${pageContext.request.queryString}" class="glyphicon glyphicon-remove-sign"></a></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
					</div>
					<button type="button" class="btn btn-primary btn-md pull-right" data-toggle="modal" data-target="#add">Add Country</button>
				</div>
			</div>
		</div>
		<div class="row">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="form-style-8">
					<form:form action="/admin/countries" method="post" modelAttribute="countrySaveForm" id="save">
						<form:hidden path="id" />
						<form:errors path="name" />
						<form:input path="name" placeholder="Country name" required="true" maxlength="50" />
					</form:form>
				</div>
			</div>
			<div class="modal-footer">
				<button type="submit" class="btn btn-primary" form="save" formaction="/admin/countries?${pageContext.request.queryString}">
					<span class="glyphicon glyphicon-ok"></span> Save
				</button>
				<c:choose>
					<c:when test="${countrySaveForm.id ne 0}">
						<a href="/admin/countries?${pageContext.request.queryString}" id="close" class="btn btn-default">Close</a>
					</c:when>
					<c:otherwise>
...
<c:choose>
	...
	<c:otherwise>
		<spring:hasBindErrors name="countrySaveForm">
